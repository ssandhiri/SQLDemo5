...
select A.SponsorID,
A.Sponsor,
cast(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(A.sponsor,'&',' and '),'/',' '),';',' ') ,'.','') ,',','') 
       ,':',' ') ,'@',' '),'Doctor ','Dr '),' 1 ',' One ') ,'  ',' '),' LLC',', LLC.') ,' Co ',' Co. ') ,' Ltd',', Ltd.'),'é','e'),'Assoc ','Association ')as Varchar(510))  as Sponsor_N,
A.Address1,
cast(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(A.Address1,'.',' '),',',' '),'South ','S '),'West','W '),'East','E '),'North','N ')
,'Avenue','Ave') ,'#','Suite') ,' Ste',' Suite')  ,'Two','2') ,'One ','1 ') ,'Three ','3 ') ,'Building',' Bldg'), char(9),' '), char(10),' '), char(13),' ') ,'  ',' '),'  ',' ')as Varchar(75))  as Address1_N,

A.Address2,
cast(replace(replace(replace(replace(replace(replace(replace(replace(replace(A.Address2,'.',' '),',',' '),'United Kingdom','UK'),'First','1st'),' St ',' Street '),'#',''),' Ste ',' Suite '),' Fl ',' Floor '),'  ',' ')as Varchar(75)) as Address2_N,
A.City,cast(Ltrim(Rtrim(case when A.City is null then '' else A.city end ))as Varchar(50)) As City_N,
A.State,cast(Ltrim(Rtrim(case when A.State is null then '' else A.state end ))as Varchar(2)) As State_N,
A.Zip,
cast(ltrim(rtrim(
       case when rtrim(ltrim(A.Zip)) is null then '' 
              ...
              when rtrim(ltrim(A.Zip)) like '%-%' then dbo.UFN_SEPARATES_COLUMNS(rtrim(ltrim(A.Zip)), 1, '-')
                     else rtrim(ltrim(A.Zip)) end))as Varchar(10))  as Zip_N,
cast(ltrim(rtrim(
...
PATINDEX('% %', ltrim(substring(rtrim(ltrim(Zip)),PATINDEX('% %', rtrim(ltrim(Zip))),len(rtrim(ltrim(Zip)))))) )
       ...
       when rtrim(ltrim(A.Zip)) like '% %' and len(rtrim(ltrim(Zip))) <9    then substring(rtrim(ltrim(A.Zip)),PATINDEX('% %', rtrim(ltrim(A.Zip))),len(rtrim(ltrim(A.Zip))))
              ...
              when rtrim(ltrim(A.Zip)) like '%-%' then dbo.UFN_SEPARATES_COLUMNS(rtrim(ltrim(A.Zip)), 2, '-')
                     else '' end))as Varchar(11))  as Zip_Suffix_N,
cast((Case when A.State='UK' then 'UK' else B.Country end )as Varchar(40)) as Country_N,
A.CurrentSponsor,
cast(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(A.currentSponsor,'&',' and '),'/',' ') ,'.',''),',','') 
,';',' ') ,':',' ') ,'@',' '),'Doctor ','Dr '),' 1 ',' One ') ,'  ',' '),' LLC',', LLC.'),' Co ',' Co. ') ,' Ltd',', Ltd.') ,'é','e'),'Assoc ','Association ')as Varchar(255)) as CurrentSponsor_N
...
;
  select SponsorID,
       ...
       City,State,Zip,
       CurrentSponsor,
       cast(nullif(rtrim(ltrim(case when sponsor_N like '% Inc%' and sponsor_N not like '% Inco%' then replace(sponsor_N,' Inc',', Inc.') 
              else sponsor_N end)) ,'')  as Varchar(510)) as Sponsor_N,
...
drop table [dbo].[tblSponsor_Golden_Master]
